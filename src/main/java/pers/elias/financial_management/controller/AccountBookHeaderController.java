...
public class AccountBookHeaderController {
    ...
    public String uploadAccountBookHeader(MultipartFile file, HttpServletRequest request) {
        ...
        try {
            //设置保存路径
            String dirPath = "D:\\work-space\\financial_management_elias\\src\\main\\resources\\static\\images";
            ...
            //设置相对路径
            String relativePath = "/images/" + fileUpload.getFilename();
            //账本头像实体类
            AccountBookHeader accountBookHeader = new AccountBookHeader();
            accountBookHeader.setUserName(globalAccountInfo.getUserName());
            accountBookHeader.setAccountBookId(globalAccountInfo.getAccountBookId());
            ...
            if(accountBookHeaderService.isExists(accountBookHeader)){
                ...
                aBH.setAccountBookId(globalAccountInfo.getAccountBookId());
                //查询现有路径  获取路径后缀文件名称
